...
class AnnotationOverlay: NSView {
    ...
    override func menu(for event: NSEvent) -> NSMenu? {
        ...
        if let id = hitArrowBody(at: loc) ?? hitArrowTail(at: loc) {
            ...
            let del = NSMenuItem(title: "删除箭头", action: #selector(deleteSelectedItem(_:)),
                                 keyEquivalent: "")
        ...
        for ann in textAnnotations.reversed() {
            ...
            if hitRect.contains(loc) {
                ...
                let edit = NSMenuItem(title: "编辑文字", action: #selector(editSelectedText(_:)),
                                      keyEquivalent: "")
                edit.target = self; menu.addItem(edit)
                let del = NSMenuItem(title: "删除文字", action: #selector(deleteSelectedItem(_:)),
                                     keyEquivalent: "")
                del.target = self; menu.addItem(del)
                addArrangeItems(to: menu)
                return menu
        ...
        for shape in shapes.sorted(by: { $0.zOrder > $1.zOrder }) {
            if viewRect(for: shape.rect).standardized.contains(loc) {
                ...
                let del = NSMenuItem(title: "删除形状／遮挡", action: #selector(deleteSelectedItem(_:)),
                                     keyEquivalent: "")
                del.target = self; menu.addItem(del)
                addArrangeItems(to: menu)
                return menu
        ...
        for region in blurRegions.sorted(by: { $0.zOrder > $1.zOrder }) {
            if viewRect(for: region.rect).standardized.contains(loc) {
                ...
                let del = NSMenuItem(title: "删除模糊", action: #selector(deleteSelectedItem(_:)),
                                     keyEquivalent: "")
                del.target = self; menu.addItem(del)
                addArrangeItems(to: menu)
                return menu
        ...
        for h in highlights.sorted(by: { $0.zOrder > $1.zOrder }) {
            if viewRect(for: h.rect).standardized.contains(loc) {
                ...
                let del = NSMenuItem(title: "删除高亮", action: #selector(deleteSelectedItem(_:)),
                                     keyEquivalent: "")
                del.target = self; menu.addItem(del)
                addArrangeItems(to: menu)
                return menu
        ...
        for s in spotlights.sorted(by: { $0.zOrder > $1.zOrder }) {
            if viewRect(for: s.rect).standardized.contains(loc) {
                ...
                let del = NSMenuItem(title: "删除聚焦", action: #selector(deleteSelectedItem(_:)),
                                     keyEquivalent: "")
                del.target = self; menu.addItem(del)
                addArrangeItems(to: menu)
                return menu
        ...
        for badge in stepBadges.sorted(by: { $0.zOrder > $1.zOrder }) {
            ...
            if hypot(loc.x - center.x, loc.y - center.y) <= badge.diameter / 2 + 4 {
                ...
                let del = NSMenuItem(title: "删除编号", action: #selector(deleteSelectedItem(_:)),
                                     keyEquivalent: "")
                del.target = self; menu.addItem(del)
                addArrangeItems(to: menu)
                return menu
        ...
        // Generic copy option.
        let copy = NSMenuItem(title: "复制编辑结果", action: #selector(copyImageAction(_:)),
                              keyEquivalent: "")
    ...
    private func addArrangeItems(to menu: NSMenu) {
        ...
        let fwd  = NSMenuItem(title: "上移一层",  action: #selector(bringForward(_:)),  keyEquivalent: "")
        let back = NSMenuItem(title: "下移一层",  action: #selector(sendBackward(_:)),  keyEquivalent: "")
        let top  = NSMenuItem(title: "置于顶层", action: #selector(bringToFront(_:)),  keyEquivalent: "")
        let bot  = NSMenuItem(title: "置于底层",   action: #selector(sendToBack(_:)),    keyEquivalent: "")
    ...
    var selectedRedaction: (pixelated: Bool, color: NSColor, intensity: CGFloat)? {
        if let region = document?.blurRegions.first(where: { $0.id == selectedBlurID }) {
            return (true, currentFillColor, region.intensity)
        }
        if let shape = document?.shapes.first(where: { $0.id == selectedShapeID && $0.borderWeight == 0 && $0.fillColor.alphaComponent == 1 }) {
            return (false, shape.fillColor, currentBlurIntensity)
        }
        return nil
    }

    func changeSelectedRedaction(pixelated: Bool, color: NSColor, intensity: CGFloat) {
        guard let id = selectedBlurID ?? selectedShapeID, let document else { return }
        document.changeRedaction(id: id, pixelated: pixelated, color: color, intensity: intensity)
        if pixelated, document.blurRegions.contains(where: { $0.id == id }) {
            activeTool = .blur
            selectedBlurID = id
        } else if !pixelated, document.shapes.contains(where: { $0.id == id && $0.borderWeight == 0 }) {
            activeTool = .shape
            selectedShapeID = id
        }
        needsDisplay = true
